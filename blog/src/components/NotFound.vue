<template>
  <div class="body">
    ...
    <div id="info">
        <p>您访问的页面不存在， 或者您输入的网址有误！</p>
        <p><span>{{num}}</span>秒后自动跳转到主页！</p>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$store.commit('setTitle', '404');
    this.timer(1000);
    $('.top-bar-container').css('visibility', 'hidden');
  },
  destroyed() {
    $('.top-bar-container').css('visibility', 'visible');
  }
}
</script>
